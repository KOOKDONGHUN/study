...
model = XGBClassifier(n_estimators=80,learning_rate=0.1)
# model.fit(x_train,y_train, verbose=True, eval_metric='error',eval_set=[(x_train, y_train), (x_test, y_test)])
model.fit(x_train,y_train, verbose=True, eval_metric=['rmse','logloss','auc'],eval_set=[(x_train, y_train), (x_test, y_test)])#, early_stopping_rounds=)
