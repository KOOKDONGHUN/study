...
parser.add_argument('--model_params',
                    type=str,
                    default=r'C:\Users\Administrator\KoGPT2-chatbot\model_chp\model_last.ckpt',
        ...
        with torch.no_grad():
            ...
            while 1:
                ...
                while 1:
                    ...
                    pred = self(input_ids)
                    total = self.vocab.to_tokens(
                        torch.argmax(
                            pred,
                            dim=-1).squeeze().numpy().tolist())
                    gen = total[-1]
                    if gen == EOS or len(total)>=200:
                        break
                    elif (des_t - st_t) > 100 :
                        break
                    a += gen.replace('▁', ' ')
...
def show_page():
    ...
    return render_template('home.html', question = text, answer = ans)
