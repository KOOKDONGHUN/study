...
train = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.01).minimize(cost) # 경사하강법 인데 최소의 cost를 찾겠다 
...
with tf.Session() as sess: # 전체가 범위안에 포함된다? 이해안됨 안에있는 세션이 다 실행한다 ??
    ...
    # print(sess.run(w))
    # print(sess.run(b))
    # sess.run(x_train,feed_dict= {x_train: [1, 2, 3]})
    # sess.run(y_train,feed_dict= {y_train : [3, 5, 7] })
