...
w6 = tf.Variable(tf.random_normal([32, 10]), name='weight6')
b6 = tf.Variable(tf.random_normal([10]),name = 'bias6')
h = tf.nn.softmax(tf.matmul(layer5, w6) + b6)

loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(h),axis=1)) # loss ... 계산 방법 ...

opt = tf.train.GradientDescentOptimizer(learning_rate=1).minimize(loss) # 어떻게 쓰는지 어떻게 계산하는지 지금은 일단 쓰고 시간이 많을때 꼭!!! 공부하라 경사하강법
