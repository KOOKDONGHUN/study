...
def autoencoder(hidden_layer_size):
    model = Sequential()
    model.add(Conv2D(filters=hidden_layer_size, kernel_size=(3,3), padding='valid',input_shape=(28,28,1),activation='elu'))
    model.add(MaxPooling2D((2,2), padding='same'))

    model.add(Conv2D(filters=64, kernel_size=(3,3), padding='valid',activation='elu'))

    model.add(Conv2DTranspose(64, kernel_size=(3,3),padding='valid',activation='elu'))
...
model.compile(optimizer='adam',loss='binary_crossentropy', metrics=['acc']) # loss: 0.0932

model.fit(x_train_noised,x_train,epochs=5,validation_split=0.2)
