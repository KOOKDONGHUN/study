...
from keras.layers import Dense, Conv2D, LSTM , Flatten, Dropout, MaxPooling2D,Input, BatchNormalization, Activation
...
''' 0과1 배단 함수? -> 중간값도 주자 -> 시그모이드 
    그래프상으로는 val_acc가 train 보다 지속적으로 좋게나옴 하지만
    loss가 어느정도 내려가서 더이상 내려가지 않는 것을보니 epoch를 늘려도 별로 효과없을듯 보임
    acc 0.676 
    epochs 30 -> 40 acc:0.6834 loss는 더이상 낮아지지않음 val_acc도 높아지고 있는것처럼 보임
    epochs 40 -> 50 acc:0.66 '''
...
model = Sequential()
model.add(Conv2D(nd**4,kernel_size=ft,padding=pd ,input_shape=(x_train.shape[1],x_train.shape[2],x_train.shape[3])))
model.add(BatchNormalization())
model.add(Activation(act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))

model.add(Conv2D(nd**5,kernel_size=ft,padding=pd))
model.add(BatchNormalization())
model.add(Activation(act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))

model.add(Conv2D(nd**6,kernel_size=ft,padding=pd))
model.add(BatchNormalization())
model.add(Activation(act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))
...
hist = model.fit(x_train,y_train,epochs=50,batch_size=32,callbacks=[els],verbose=2,validation_split=0.01)
