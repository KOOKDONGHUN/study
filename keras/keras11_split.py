...
model.fit(x_train,y_train,epochs=168, batch_size=7,
            validation_data=(x_val,y_val)) # batch_size = 32(default)

#4. 평가, 예측
loss,mse = model.evaluate(x_test,y_test,batch_size=7) # evaluate -> 결과 반환(기본적으로 loss와 metrics를 반환)을 loss와 acc에 받겠다.
