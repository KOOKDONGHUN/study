...
# dropout 3개 과적합을 피할수 있는가? -> l2보다 좋음 0.77

model = Sequential()
model.add(Conv2D(nd**7,kernel_size=ft,padding=pd ,input_shape=(32,32,3),activation=act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))
model.add(Dropout(0.1))


model.add(Conv2D(nd**8,kernel_size=ft,padding=pd,activation=act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))
model.add(Dropout(0.1))

model.add(Conv2D(nd**9,kernel_size=ft,padding=pd,activation=act))
model.add(MaxPooling2D(pool_size=ft,strides=2,padding=pd))
model.add(Dropout(0.1))
