...
# 3. 컴파일(훈련준비),실행(훈련)
modelpath = './model/Sample/model/BOSTON/{epoch:02d}--{val_loss:.4f}.hdf5'
...
hist = model.fit(x_train,y_train,epochs=15,batch_size=3,callbacks=[chpoint],verbose=2,validation_split=0.03)

model.save('./model/Sample/model/BOSTON/keras73_model.h5') # 가중치 까지 저장됨
model.save_weights('./model/Sample/model/BOSTON/keras73_weight.h5') # 가중치만 저장됨
