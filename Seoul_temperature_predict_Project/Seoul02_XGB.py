...
for j in range(len(y_data[0,:])) :
    ...
    for i in range(len(y_pred)):
        if 6+i <13:
            print(f"2019-{6+i}\t y_tets : {y_test[i]} \t y_pred : {np.round(y_pred[i],1)}")
        else:
            print(f"2020-{i-6}\t y_tets : {y_test[i]} \t y_pred : {np.round(y_pred[i],1)}")
        # pass

    r2_y_pred = r2_score(y_test[:, j],np.round(y_pred,1))
    print("r2 : ",r2_y_pred)
