...
print(data) # 
print(len(data)) # 2
print(len(data[1])) # 64 // batch_size=64 이기 때문에 64개씩 잘라짐 // 이래서 나중에 속도가 빠르다고 하나봄

img, label = data 

print(img.size()) # torch.Size([64, 1, 28, 28]) 테이터가 케라스랑 다르게 채널이 마지막이 아니고 2번째에 있네 
print(label.size()) # torch.Size([64])
...
class CNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        for d in out.size()[1:]: #16, 4, 4
            dim = dim * d
        out = out.view(-1, dim) # keras Flatten
