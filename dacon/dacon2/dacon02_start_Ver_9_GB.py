...
def model_fit(y) :
    ...
    data = model.predict(test_features)
    df = pd.Series(data)
    return df
print("train_target",train_target.shape)

X = model_fit(train_target[train_target.columns[0]])
Y = model_fit(train_target[train_target.columns[1]])
M = model_fit(train_target[train_target.columns[2]])
V = model_fit(train_target[train_target.columns[3]])
...
df.to_csv('./data/dacon/comp2/sample_submission_GB.csv', index = True, header=['X','Y','M','V'],index_label='id')